...
exports.predictLoanController = async (req, res) => {
    try {
        ...
        const result = await aiService.predictLoan({ income, expenses, savings, loan_amount, credit_score, employment_status, transaction_history });
        res.json(result);
    } catch (err) {
        console.error('[AI Controller] predictLoan error:', err);
        res.status(500).json({ success: false, message: 'AI Engine not available', error: err.message });
    }
};
...
exports.detectFraudController = async (req, res) => {
    try {
        ...
        if (!amount || amount <= 0) {
            return res.status(400).json({ msg: 'Enter a valid transaction amount' });
        }
        const result = await aiService.detectFraud({ amount, location, device, frequency, transaction_time });
        res.json(result);
    } catch (err) {
...
exports.predictSavingsController = async (req, res) => {
    try {
        ...
        const result = await aiService.predictSavings({ income, expenses, savings, age, dependents, employment_type });
        res.json(result);
    } catch (err) {
...
exports.analyzeSpendingController = async (req, res) => {
    try {
        ...
        if (!transactions || !Array.isArray(transactions)) {
            return res.status(400).json({ msg: 'Transactions needed' });
        }
...
exports.recommendationController = async (req, res) => {
    try {
        ...
        const result = await aiService.getRecommendations({ income, expenses, savings, age, risk_tolerance, goals, dependents, employment_type });
        res.json(result);
    } catch (err) {
...
exports.marketIntelligenceController = async (req, res) => {
    try {
        const result = await aiService.getMarketIntelligence();
        res.json(result);
    } catch (err) {
        console.error('[AI Controller] marketIntelligence error:', err);
        res.status(500).json({ success: false, message: 'AI Engine not available', error: err.message });
    }
};

exports.aiDashboardController = async (req, res) => {
    try {
        const result = await aiService.getAIDashboard();
        res.json(result);
    } catch (err) {
        console.error('[AI Controller] aiDashboard error:', err);
        res.status(500).json({ success: false, message: 'AI Engine not available', error: err.message });
    }
};
